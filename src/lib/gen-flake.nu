...
def output-flake [env_names systems inputs outputs] {
    let default = (gen-one-env "envil-current" {extends: $env_names}).output
    (r
     (c "This has been generated by `envil'\nDO NOT EDIT MANUALLY")
     (a
        description (s $"envil-generated flake for env\(s) ($env_names)")
        inputs (rec2a $inputs)
        outputs (f inputs
            (let_
                ...
                (a packages (r forEachSystem (f system
                    (let_
                        [imp (r (c "An attrset containing each input's packages for the current system")
                         ...
                         envs (r (c $"Each environment used by env\(s) ($env_names)")
                                 (rec2a ({default: $default} | merge $outputs)))]
                        envs))))))))
}
...
export def generate-flake [
    ...
    output-flake $env_names $systems $inputs $outputs |
