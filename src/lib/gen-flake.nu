...
def output-flake [envnames systems nixpkgs_key inputs outputs] {
    let current = (gen-one-env "envil-current" {extends: $envnames}).output
                    ...
                    (let_
                        [imp (r (c "An attrset containing each input's packages for the current system")
                         ...
                         envs (r (c $"Each environment used by env ($envname)")
                                 (rec2a ({__current: $current} | merge $outputs)))]
                        envs))))))))
}
...
export def generate-flake [
    ...
    output-flake $envnames $systems $nixpkgs_key $inputs $outputs |
