...
Most `envil` subcommands accept a `-d` flag to specify which statedir to use. The statedir passed to `envil` with `-d` is always registered,
and the aforementioned subcommands will use the last statedir when used without `-d`.
...
- `envil switch -m`: let you select multiple (`-m`) environments and replace the whole stack with them
- `envil update`: update the flake.lock files for some environments, and refresh the current stack
- `envil status`: show the currently activated env, the last used statedir, and (if any) the env activated in the current subshell
